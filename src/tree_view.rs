...
use crate::{Pm, BAR, BORDER, CHANGED, DIM, ICON_SIZE, PANEL, SELECT, TEXT, TREE_INDENT, TREE_ROW_H};
    ...
    fn prepaint(
        ...
        let (first, off_y, count, hover, rows) = self.pm.update(cx, |pm, _cx| {
            ...
            for vi in first..last {
                let e = &pm.tree[pm.visible[vi]];
                let changed = pm.changed.contains(&e.rel);
                let selected = pm.tree_selected.as_deref() == Some(e.rel.as_path());
                let expanded = e.is_dir && pm.expanded.contains(&e.rel);
...

        {
            ...
            window.on_mouse_event(move |e: &MouseDownEvent, phase, window, cx| {
                ...
                if let Some(vi) = row_at(window, pos, off_y) {
                    pm.update(cx, |pm, cx| {
                        let idx = pm.visible[vi];
                        let rel: PathBuf = pm.tree[idx].rel.clone();
                        if pm.tree[idx].is_dir {
                            if !pm.expanded.remove(&rel) {
                                pm.expanded.insert(rel.clone());
                            }
                            pm.tree_selected = Some(rel);
                            pm.rebuild_visible();
                        } else {
