...
impl Element for ListView {
    ...
    fn prepaint(
        ...
        let (first, off_y, count, hover, rows) = self.pm.update(cx, |pm, _cx| {
            ...
            pm.list_scroll.content = size(px(0.0), px(count as f32 * row_h));
    ...
    fn paint(
        ...
        window.with_content_mask(Some(ContentMask { bounds }), |window| {
            ...
            for (k, row) in p.rows.iter().enumerate() {
                ...
                row.status
                    .paint(point(px(left + 10.0), px(y)), px(row_h), TextAlign::Left, None, window, cx)
                    .ok();

                let (ikey, idata) = row.icon;
                window
                    .paint_svg(
                        sq(left + 24.0, y, icon_sz),
                        ikey.into(),
                        Some(idata),
                ...
                row.name
                    .paint(
                        point(px(left + 24.0 + icon_sz + 8.0), px(y)),
                        px(row_h),
                        TextAlign::Left,
                        None,
                        window,
                ...
                if let Some(badge) = &row.badge {
                    ...
                    badge
                        .paint(point(px(bx), px(y)), px(row_h), TextAlign::Left, None, window, cx)
                        .ok();
                }
            }
